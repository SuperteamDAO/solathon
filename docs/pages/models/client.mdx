...
Returns all SPL Token accounts using the public key provided.

<Code>
```python 
def get_token_accounts_by_owner(public_key: PublicKey | str)
```
</Code>

#### .send_transaction
Submits a signed transaction to the cluster for processing, recent blockhash is optional.
